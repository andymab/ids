    ...
    <v-dialog v-model="showCropper" max-width="500" persistent>
      <v-card class="pt-6 pb-3">
        ...
        <v-card-text class="pb-3">
          <vue-cropper ref="cropper" class="image-container" :aspect-ratio="16 / 9" :guides="true" :background="false"
            :view-mode="3" drag-mode="move" :src="chosenImage" alt="Image not available">
          </vue-cropper>
        </v-card-text>
        <v-card-actions class="py-0 mx-10">
          <v-btn @click="resetCropper" text color="red"> Cancel </v-btn>
          <v-spacer></v-spacer>
          <v-btn @click="cropChosenImage" text color="blue"> Crop </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
